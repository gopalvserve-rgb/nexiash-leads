...
const routes = {
  ...
  // Nexiash: Education industry pack — fees, installments, reminders, students, parents
  education:     require('./routes/packs/education')
};
const webhooks = require('./routes/webhooks');
const eduPackFramework = require('./routes/packs/_framework');
...
(async () => {
  console.log(`[boot] starting Nexiash CRM on ${HOST}:${PORT} (node ${process.version})`);
  console.log(`[boot] DATABASE_URL set: ${!!process.env.DATABASE_URL}`);
  console.log(`[boot] JWT_SECRET set: ${!!process.env.JWT_SECRET}`);
  await bootstrap();

  // Nexiash: auto-install the Education industry pack on boot.
  // The framework is idempotent — only runs the installer if not already active.
  try {
    const active = await eduPackFramework.isPackActive('education');
    if (!active) {
      console.log('[boot] installing Education industry pack…');
      await eduPackFramework.installPack('education', { userId: null });
      console.log('[boot] ✓ Education pack installed');
    } else {
      console.log('[boot] Education pack already active');
    }
  } catch (e) {
    console.error('[boot] Education pack install failed:', e.message);
  }
